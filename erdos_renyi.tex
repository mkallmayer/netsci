...
intuitiv hat \qq{jeder zweite Graph} mindestens die Hälfte aller möglichen Kanten.
...
E.~Gilbert \aside{Gilbert-Graphen \Gnp} beschreibt ein ähnliches Modell, das \Gnp-Modell -- das oft fälschlicherweise \qq{Erd\H{o}s-R\'enyi-Modell} genannt wird.
...
\begin{itemize}
    ...
    \item Beobachtete Graphen haben oft \qq{Rauschen}, \dh nicht verstandene oder insignifikante Strukturen, die Messungen verfälschen können.
...
\noindent Da in verschiedenen wissenschaftlichen Communities der Begriff \qq{Generator} unterschiedlich benutzt wird, hier noch eine Definition:
\begin{definition}
...
\begin{lemma}
    \label{lem:naive-gnp}
    \Cref{alg:naive-gnp} erzeugt einen \Gnp-Graphen in $\Theta(n^2)$ Zeit.
\end{lemma}
...
Es wäre schön, wenn wir diese Graphen schneller erzeugen könnten, jedoch ist \cref{alg:naive-gnp} für alle $p$ gleich \qq{schnell}.
...
Aber woher wissen wir, wo \qq{Nullen} gezogen werden, bzw. wie viele Nullen wir überspringen müssen?
...
\begin{lemma}
    ...
    \Cref{alg:linear-gnp} erzeugt Graph $G = (V, E) \follows \Gnp$ in $\Theta(|E|)$ Zeit.
\end{lemma}
...
\begin{figure}[t]
    ...
    \caption{
        ...
        \textbf{Rechts}: \qq{Umkehrfunktion} $F^{-1}(p)$ der kumulativen Verteilungsfunktion $F(x)$.
    }
...
\begin{algorithm}[H]
    \KwIn{Datenstrom~$S$, Stichprobengröße~$k$}
...
\begin{algorithm}[H]
    \KwIn{Universum~$S$, Stichprobengröße~$k$}
...
Angenommen, wir führen gerade Task $t_0$ aus, dann können wir die Berechnung in zwei \qq{Kindertasks} $t_1$ und $t_2$ tei\-len \aside{fork} (\emph{forken}).
...
$t_1$ und $t_2$ werden \qq{gelöscht} und $t_0$ wieder geweckt, der seine Berechnung fortsetzt.
...
\begin{enumerate}
    \item
          ...
          Der \aside{arbeitsoptimal} Algorithmus leistet also asymptotisch nicht mehr Arbeit als die Laufzeit der besten sequentiellen Lösung; wir sagen, er ist \emph{arbeitsoptimal}.
...
Analog zu den zufälligen Sprüngen in \cref{subsec:gnp_zufaellige_spruenge} ziehen wir $k_1$ aus einer hypergeometrischen Verteilung.
...
\begin{algorithm}
    ...
    \Fn{\ParSelect{$S=\set{s_1, \ldots, s_N}, k$}}{
        ...
        \BlankLine
        \Fork{$
                \underbrace{\ParSelect{$S_1 = \set{s_1, \ldots, s_{N_L}}, k_L$}}_{\text{Task $t_1$}}, \ \
                \underbrace{\ParSelect{$S_2 = \set{s_{N_L + 1}, \ldots, s_N}, k - k_L$}}_{\text{Task $t_2$}}
            $}\;
...
Wie in \cref{fig:benchmark_gnm_recursive_scale} zeigt, weist eine (sequentielle) Implementierung von \cref{algo:parallel_k_aus_N} ein gutes Skalierungsverhalten auf.
...
\begin{lemma}[basiert auf \cite{DBLP:journals/rsa/KrivelevichS13}]\label{lemma:dfs_kaum_einsen_in_X}
    Sei $\epsilon > 0$ \aside{kleines $\epsilon > 0$ \\ $N = \binom n 2$ \\ $p = (1 - \epsilon) / n$ \\ $k = (10 / \epsilon^2) \ln n$} eine hinreichend kleine Konstante und seien $X = (X_1, \ldots, X_N)$ unabhängige Zufallsvariablen, die Bernoulli-verteilt mit Parameter~$p$ sind.
    %\begin{enumerate}
    %\item
...
Weiter ist die Abhängigkeit der ersten beide Aussagen von $\epsilon^2$ optimal.
...
Zuletzt wollen wir uns noch eine Beweistechnik ansehen, bei der wir zwei geeignet parametrisiert \Gnp-Graphen durch Vereinigung ihrer Kantenmengen \qq{aufaddieren}.
